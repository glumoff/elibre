...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/ 
  ...
  #       forceStretch can be applied to options 1,2,3 and 4  
    ...
    if (!$this->image) { if ($this->debug) { return; die('file ' . $this->getFileName() .' is missing or invalid (file: '.__FILE__.', line #'.__LINE__.')'); }else{ die(); }};
...
## -------------------------------------------------------- 
  ...
  #       third party class to save as bmp.
    ...
    if (!is_resource($this->imageResized)) { if ($this->debug) { return; die('saveImage: This is not a resource.'); }else{ die(); }}      
    $fileInfoArray = pathInfo($savePath);
    clearstatcache();
    if (!is_writable($fileInfoArray['dirname'])) {  if ($this->debug) { return; die('The path is not writable. Please check your permissions.'); }else{ die(); }} 
